...
\begin{quote}
	...
	If the problem has dimensions $m \times 2$,
	then there are two possibilities:
	either the maximum of the central column is a peak, which means it is the best neighbor and better value that has been seen so far by the algorithm (in which case the algorithm will halt and return the location), or it has a strictly better neighbor in the other column
	(in which case the algorithm will determine if this neigbor is better then the best value seen so far and recurse
	on the non-empty subproblem with dimensions $m \times 1$, which contains the best value seen so far, thus reducing to the previous case). Same logic follows for the cases with the dimenstions $1 \times n$ or $2 \times n$
	...
	then that location must have the best value in column $c_1$ and be the best value that has been seen by the algorithm,
	and must have been a peak within some recursive subproblem.
	Assume, for the sake of contradiction,
	that $(r_1, c_1)$ is not also a peak within the original problem.
	Then as the location $(r_1, c_1)$ is passed up the chain of recursive calls,
	it must eventually reach a level where it stops being a peak.
	At that level, the location $(r_1, c_1)$
	must be adjacent to the dividing column $c_2$ (where $|c_1 - c_2| = 1$), or to the dividing row $r_2$ (where $|r_1 - r_2| = 1$). Without loss of generality let's consider only the case with dividing column $c_2$. In that case the values must satisfy the inequality
	$val(r_1, c_1) < val(r_1, c_2)$.
	...
	Because the algorithm chose to recurse
	on the half containing $(r_1, c_1)$,
	we know that there is some value $v$ that is strictly greater than  $val(r_2, c_2)$. This value is the best seen value by the algorithm at that point.
	Hence, we have the following chain of inequalities:
	$$val(r_1, c_1) < val(r_1, c_2) < v$$
	But in order for \texttt{algorithm4} to return $(r_1, c_1)$ as a peak,
	the value at $(r_1, c_1)$ must have been the greatest value seen by the algorithm,
	making $val(r_1, c_1) > val(r_2, c_1)$.
	Hence, we have a contradiction.
\end{quote}
