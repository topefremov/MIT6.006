# -*- coding: utf-8 -*-
"""
Created on Mon Dec 16 16:23:47 2019
...
class PriorityQueue:
    ...
    def append(self, key):
        ...
        while i > 0 and self.queue[PriorityQueue._parent(i)] > self.queue[i]:
            self.queue[i], self.queue[PriorityQueue._parent(i)] = self.queue[PriorityQueue._parent(i)], self.queue[i]
    ...
    def _min_heapify(self, pos):
        ...
        largest = pos
        if left is not None and self.queue[left] < self.queue[pos]:
            largest = left
        if right is not None and self.queue[right] < self.queue[largest]:
            largest = right
        if largest != pos:
            self.queue[pos], self.queue[largest] = self.queue[largest], self.queue[pos]
            self._min_heapify(largest)
